class Communication < ActiveRecord::Base
	...
	belongs_to :client
	serialize :to
  validates_email_format_of :cc, :message => 'Email is not looking good' 
  #validates_email_format_of :to, :message => 'Email is not looking good' 
	#Get data according to date
  def self.emails_communication_summary(report)
    date = Date.today
